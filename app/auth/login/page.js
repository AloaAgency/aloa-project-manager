  ...
  const handlePasswordLogin = async (e) => {
    ...
    try {
      ...
      if (!response.ok || result.error) {
        ...
        } else if ((userRole === 'client' || userRole === 'client_admin' || userRole === 'client_participant') && result.clientProject) {
          // All client type users go to their project dashboard if they have a project
          console.log(`Redirecting ${userRole} to project dashboard:`, result.clientProject.id);
          window.location.href = `/project/${result.clientProject.id}/dashboard`;
        } else if (userRole === 'client' || userRole === 'client_admin' || userRole === 'client_participant') {
          // Client users without a project go to a waiting page
          console.log(`${userRole} has no project, redirecting to dashboard`);
          window.location.href = '/dashboard';
        } else {
          // Default fallback
          console.log('Default redirect to dashboard for role:', userRole);
          window.location.href = '/dashboard';
        }
      } else {
