...
export function useProjectKnowledge(projectId, contextType = 'full_project') {
  ...
  const fetchProjectKnowledge = async () => {
    try {
      ...
      console.error('Error fetching project knowledge:', err);
  ...
  const refreshKnowledge = async () => {
    try {
      ...
      console.error('Error refreshing project knowledge:', err);
      setError(err.message);
      throw err;
    }
  };
  ...
  const formatForAI = () => {
    ...
    for (const [category, items] of Object.entries(knowledge.knowledge || {})) {
      ...
      for (const item of items) {
        formatted += `- ${item.source} (Importance: ${item.importance}/10)\n`;
