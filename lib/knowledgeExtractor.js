  ...
  async extractFromAppletInteraction(interactionId) {
    try {
      ...
      if (interaction.applet?.type === 'tone_of_voice') {
        const selectedTones = interactionData.selectedTones || [];
        ...
        if (selectedTones.length > 0) {
          knowledgeItems.push({
            ...
            content_type: 'preferences',
            content: JSON.stringify(selectedTones),
            content_summary: `Client selected brand voice: ${selectedTones.join(', ')}`,
            category: 'content_strategy',
            tags: ['tone', 'voice', 'brand_personality', 'content'],
            importance_score: 8,
            extracted_by: 'system',
            extraction_confidence: 1.0,
            processed_at: new Date().toISOString(),
  ...
  async extractFromFile(fileId) {
    try {
      ...
      if (file.mime_type === 'text/plain' || file.mime_type === 'text/markdown') {
        ...
        summary = `Document: ${file.file_name} (${file.mime_type})`;
        content = summary;
      }
      ...
      const knowledgeItem = {
        ...
        category: this.categorizeFile(file.file_name, content),
        tags: this.extractTags(file.file_name, content),
        importance_score: 6,
        extracted_by: 'system',
