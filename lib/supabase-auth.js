...
import { createClient as createSupabaseBrowserClient } from './supabase-browser';
export const createClient = createSupabaseBrowserClient;

const getRedirectOrigin = (fallback = 'http://localhost:3001') => {
  if (typeof window !== 'undefined') {
    // In browser, prefer window.location.origin for flexibility
    return window.location.origin ||
           process.env.NEXT_PUBLIC_APP_URL ||
           fallback;
  }
  // On server, use configured APP_URL
...
export async function resetPassword(email) {
  ...
  const { data, error } = await supabase.auth.resetPasswordForEmail(email, {
    redirectTo: `${getRedirectOrigin()}/auth/verify-reset`  // Changed to verify page
  });
