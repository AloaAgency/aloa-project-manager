-- Complete setup for aloa_forms with ALL functionality
-- This ensures the aloa_forms system has feature parity with the legacy system

-- 1. Add ALL necessary columns to aloa_forms table
...
-- 2. Ensure aloa_form_fields has all necessary columns
...
-- 3. Ensure aloa_form_responses has all necessary columns
...
-- 5. Fix ALL foreign key constraints
...
-- 7. Create function to get form with all related data
CREATE OR REPLACE FUNCTION get_aloa_form_complete(form_id UUID)
RETURNS JSON AS $$
DECLARE
    result JSON;
BEGIN
    SELECT json_build_object(
        ...
        'fields', COALESCE(json_agg(
            json_build_object(
                ...
                'order', ff.field_order,
                'help_text', ff.help_text,
                'default_value', ff.default_value
            ) ORDER BY ff.field_order
        ) FILTER (WHERE ff.id IS NOT NULL), '[]'::json),
        'response_count', COUNT(DISTINCT r.id)
    ) INTO result
    FROM aloa_forms f
    LEFT JOIN aloa_form_fields ff ON ff.aloa_form_id = f.id
    LEFT JOIN aloa_form_responses r ON r.aloa_form_id = f.id
...
-- 8. Clear any orphaned references
UPDATE aloa_applets 
SET form_id = NULL, 
    config = jsonb_set(
...
-- 9. Create triggers for auto-updating counts
...
SELECT 
    ...
    'All functionality enabled' as message,
    COUNT(*) as existing_forms_count
FROM aloa_forms;