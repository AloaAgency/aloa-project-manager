...
CREATE TABLE IF NOT EXISTS aloa_knowledge_relationships (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS aloa_ai_context_cache (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  project_id UUID NOT NULL REFERENCES aloa_projects(id) ON DELETE CASCADE,
...
-- 9. Create triggers for automatic knowledge extraction
...
-- 11. Function to search knowledge base
...
BEGIN
  ...
  SELECT
    ...
    similarity(
      COALESCE(k.content, '') || ' ' || COALESCE(k.content_summary, ''),
      p_search_terms
    ) AS relevance_score
  FROM aloa_project_knowledge k
  WHERE k.project_id = p_project_id
