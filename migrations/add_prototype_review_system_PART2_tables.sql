-- Migration: Add Prototype Review System - PART 2: Create Tables and Applet
-- Description: Creates tables and applet library entry for prototype viewer
-- Date: 2025-11-03
--
-- ⚠️  PREREQUISITE: add_prototype_review_system_PART1_enum.sql MUST be run first
-- ⚠️  Do NOT run this until PART 1 has completed successfully
--
...
-- =============================================================================
-- APPLET LIBRARY ENTRY
-- =============================================================================

-- Add Prototype Review to applet library
-- This works because the enum value was committed in PART 1
...
SELECT
  ...
  NOW()
WHERE NOT EXISTS (
  SELECT 1 FROM aloa_applet_library WHERE type = 'prototype_review'
);
...
BEGIN
  -- Check if enum value exists (should have been added in PART 1)
  SELECT EXISTS (
  ...
  SELECT EXISTS (
    ...
    WHERE type = 'prototype_review'
  ) INTO applet_library_entry_exists;

  -- Report results
  IF NOT enum_value_exists THEN
    RAISE EXCEPTION '❌ CRITICAL: prototype_review enum value does not exist. Did you run PART 1 first?';
  END IF;
